...
public class EssenctionDistillizerJEIRecipeTypeRecipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;

	public EssenctionDistillizerJEIRecipeTypeRecipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems) {
		this.output = output;
	...
	public static class Serializer implements RecipeSerializer<EssenctionDistillizerJEIRecipeTypeRecipe> {
		...
		private static final MapCodec<EssenctionDistillizerJEIRecipeTypeRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems)).apply(builder, EssenctionDistillizerJEIRecipeTypeRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, EssenctionDistillizerJEIRecipeTypeRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<EssenctionDistillizerJEIRecipeTypeRecipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, EssenctionDistillizerJEIRecipeTypeRecipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static EssenctionDistillizerJEIRecipeTypeRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new EssenctionDistillizerJEIRecipeTypeRecipe(outputs, inputs);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, EssenctionDistillizerJEIRecipeTypeRecipe recipe) {
