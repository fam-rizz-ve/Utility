...
@EventBusSubscriber
public class SporeEssenceProcedureProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, Entity sourceentity) {
		...
		if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY)
				.getEnchantmentLevel(world.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(ResourceKey.create(Registries.ENCHANTMENT, ResourceLocation.parse("utility:sporetouch")))) != 0) {
			if (Mth.nextInt(RandomSource.create(), 1, 5) == 2) {
				...
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(MobEffects.CONFUSION, 100, 10, false, false));
				createTimedLoop(5, 20, _timedLoop -> {
					if ((entity instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) != 0) {
						if (world instanceof ServerLevel _level)
							_level.sendParticles(ParticleTypes.SPORE_BLOSSOM_AIR, (entity.getX()), (entity.getY()), (entity.getZ()), 30, 1, 1, 1, 1);
					}
					return true;
				});
			}
